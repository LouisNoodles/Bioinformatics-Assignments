...
public class Rect {
	...
	public boolean contains(Rect other) {
	  ...
	  * 
	  * @pre | ball != null
	  * @post | result == null || (result.getSquareLength() == 1 && this.contains(ball.getLocation().plus(result.scaled(ball.getDiameter()/2))))
	  */
	 public Vector collideWith(Ball ball) {
	 	for (Vector coldir : COLLISSION_DIRS) {
	 		Point c = ball.getLocation().plus(coldir.scaled(ball.getDiameter()/2));
	 		if(contains(c)) {
	 			return coldir;
	 		}
	 	}
